...
public class OmronDeviceManager implements ScanController.Listener, SessionController.Listener {
    ...
    public void onScanCompletion(@NonNull @NotNull OHQCompletionReason reason) {
        registerListener.onScanCompleted();
    }

    @Override
    public void onConnectionStateChanged(@NonNull @NotNull OHQConnectionState connectionState) {
        if(registerListener != null) registerListener.onConnectionUpdate(connectionState);
    }
    ...
    @Override
    public void onLastSessionInfoUpdate(@NonNull SessionData sessionData) {
        if(registerListener == null){
            transferListener.onLastSessionInfoUpdate(sessionData);
        }else{
            registerListener.onLastSessionInfoUpdate(sessionData);
        }
    }

    @Override
    public void onDatabaseChangeIncrementUpdate(@NonNull Long databaseChangeIncrementKey) {
        if(registerListener == null){
            transferListener.onDatabaseChangeIncrementUpdate(databaseChangeIncrementKey);
        }else{
            registerListener.onDatabaseChangeIncrementUpdate(databaseChangeIncrementKey);
        }
    }

    @Override
    public void onSequenceNumberOfLatestRecordUpdate(@NonNull Integer sequenceNumberOfLatestRecord) {
        if(registerListener == null){
            transferListener.onSequenceNumberOfLatestRecordUpdate(sequenceNumberOfLatestRecord);
        }else{
            registerListener.onSequenceNumberOfLatestRecordUpdate(sequenceNumberOfLatestRecord);
        }
    }
    ...
    private void startSession(
        ...
        if (sessionController.isInSession()) {
            AppLog.i("session all ready started");
            return;
        }
    ...
    public interface RegisterListener {
        ...
        void onConnectionUpdate(OHQConnectionState state);

        void onLastSessionInfoUpdate(@NonNull SessionData sessionData);

        void onDatabaseChangeIncrementUpdate(@NonNull Long databaseChangeIncrementKey);

        void onSequenceNumberOfLatestRecordUpdate(@NonNull Integer sequenceNumberOfLatestRecord);
